...
def get_oned_bj(dtref,J,a,r=0.5,device='cpu'):
    ...
    return root_qj * torch.sqrt(dtref/torch.sqrt(a))*J
...
class Noise(object):
    ...
    def __init__(self, correlation = None, noise_size = 1):
        ...
        self.correlation = get_oned_bj if correlation is None else correlation
    ...
    def return_bj(self, grid, kappa=1):
        ...
        return self.correlation(dt/kappa,len(X),X[-1]-X[0],device=grid.device)

    def sample(self, n_sample_paths, grid, kappa=1):
        
        return torch.stack(
            [
                self._sample(n_sample_paths, grid, kappa, grid.device)
                for _ in range(self.noise_size)
            ],
            dim=1,
        )
